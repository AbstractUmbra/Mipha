CREATE TABLE IF NOT EXISTS botw_compendium (
...
CREATE TABLE IF NOT EXISTS botw_inventories (
    user_id BIGINT PRIMARY KEY,
    item_id INTEGER REFERENCES botw_compendium(item_id),
    item_level SMALLINT,
    quantity INTEGER DEFAULT 0
);
CREATE TABLE IF NOT EXISTS botw_upgrade_path (
    item_id INTEGER REFERENCES botw_compendium(item_id),
    ...
    upgrade_item_two_quantity INTEGER,
    upgrade_item_three INTEGER REFERENCES botw_compendium(item_id),
    upgrade_item_three_quantity INTEGER,
    PRIMARY KEY (
        item_id,
        item_upgrade_level
    )
