...
from utilities.shared.formats import find_nth_occurrence
...
def _create_markdown(signatures: list[str] | None, description: Iterable[Tag], url: str) -> str:
    ...
    """
    resolved_description = _get_truncated_description(
        description, markdown_converter=DocMarkdownConverter(bullets="•", page_url=url), max_length=750, max_lines=13
    )
    resolved_description = _WHITESPACE_AFTER_NEWLINES_RE.sub("", resolved_description)
...
def get_symbol_markdown(soup: BeautifulSoup, symbol_data: DocItem) -> str | None:
    ...
    if symbol_heading.name != "dt":
        assert isinstance(symbol_heading, Tag)
    ...
    return _create_markdown(signature, description, symbol_data.url).strip()  # pyright: ignore[reportArgumentType] # guarded above
