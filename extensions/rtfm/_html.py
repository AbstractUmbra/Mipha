...
class Strainer(SoupStrainer):
    ...
    def __init__(self, *, include_strings: bool, **kwargs) -> None:  # noqa: ANN003
        self.include_strings = include_strings
        passed_text = kwargs.pop("text", None)
        if passed_text is not None:
            LOGGER.warning("`text` is not a supported kwarg in the custom strainer.")
...
def get_general_description(start_element: Tag) -> list[Tag | NavigableString]:
    ...
    header = next(filter(_class_filter_factory(["headerlink"]), child_tags), None)  # type: ignore
    start_tag = header.parent if header is not None else start_element  # type: ignore
...
def get_dd_description(symbol: PageElement) -> list[Tag | NavigableString]:
    ...
    return _find_next_children_until_tag(description_tag, ("dt", "dl"), include_strings=True)  # type: ignore
...
def get_signatures(start_signature: PageElement) -> list[str]:
    ...
    for element in (
        ...
        *_find_next_siblings_until_tag(start_signature, ("dd",), limit=2),
        ...
        for tag in element.find_all(_filter_signature_links, recursive=False):  # type: ignore
            tag.decompose()
