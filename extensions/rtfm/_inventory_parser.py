...
async def _fetch_inventory(url: str, session: aiohttp.ClientSession) -> InventoryDict:
...
async def fetch_inventory(url: str, *, session: aiohttp.ClientSession) -> InventoryDict | None:
    """
    ...
    for attempt in range(1, FAILED_REQUEST_ATTEMPTS + 1):
        try:
            inventory = await _fetch_inventory(url, session=session)
        except aiohttp.ClientConnectorError:
            LOGGER.warning(
                "Failed to connect to inventory url at %s; trying again (%s/%s).", url, attempt, FAILED_REQUEST_ATTEMPTS
            )
        except aiohttp.ClientError:
            LOGGER.error("Failed to get inventory from %s; trying again (%s/%s).", url, attempt, FAILED_REQUEST_ATTEMPTS)
        except InvalidHeaderError:
            raise
        except Exception:
            LOGGER.exception(
                "An unexpected error has occurred during fetching of %s; trying again (%s/%s).",
                url,
                attempt,
