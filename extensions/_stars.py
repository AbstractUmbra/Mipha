...
def requires_starboard() -> Callable[[T], T]:
    async def predicate(ctx: Context) -> bool:
        if ctx.guild is None:
            return False

        cog: Stars = ctx.bot.get_cog("Stars")  # type: ignore # yeah idk???

        ctx.starboard = await cog.get_starboard(ctx.guild.id, connection=ctx.db)  # type: ignore # typing is gay with instance bindings
...
class StarboardConfig:
    ...
    def __init__(self, *, guild_id: int, bot: Kukiko, record: asyncpg.Record | None = None) -> None:
    ...
    @property
    def channel(self) -> MessageableGuildChannel | None:
        if self.channel_id is None:
            return

        guild = self.bot.get_guild(self.id)
        return guild and guild.get_channel(self.channel_id)  # type: ignore # so many channel types reee
...
class Stars(commands.Cog):
    ...
    def is_url_spoiler(self, text: str, url: str | None) -> bool:
    ...
    async def get_message(self, channel: MessageableGuildChannel, message_id: int) -> discord.Message | None:
        try:
            ...
            try:
                ...
                msgs = [message async for message in channel.history(limit=1, before=o)]
                msg = msgs[0]
    ...
    async def on_guild_channel_delete(self, channel: MessageableGuildChannel) -> None:
        if not isinstance(channel, (discord.TextChannel, discord.Thread, discord.VoiceChannel)):
            return

        starboard = await self.get_starboard(channel.guild.id)  # type: ignore # typing is gay with instance bindings
    ...
    async def on_raw_message_delete(self, payload: discord.RawMessageDeleteEvent) -> None:
        ...
        starboard = await self.get_starboard(payload.guild_id)  # type: ignore # typing is gay with instance bindings
    ...
    async def on_raw_reaction_clear(self, payload: discord.RawReactionClearEvent) -> None:
        ...
        channel = guild.get_channel_or_thread(payload.channel_id)
        if channel is None or not isinstance(channel, (discord.Thread, discord.TextChannel, discord.VoiceChannel)):
            return

        async with self.bot.pool.acquire(timeout=300.0) as con:
            starboard = await self.get_starboard(channel.guild.id, connection=con)  # type: ignore # typing is gay with instance bindings
    ...
    async def star_message(
        ...
        *,
        ...
        async with lock:
            async with self.bot.pool.acquire(timeout=300.0) as con:
                if verify:
                    config: Config = self.bot.get_cog("Config")  # type: ignore # yeah idk???
                    if config:
                        plonked = await config.is_plonked(guild_id, starrer_id, channel_id=channel.id, connection=con)  # type: ignore # typing is gay with instance bindings
                        if plonked:
                            return
                        perms = await config.get_command_permissions(guild_id, connection=con)  # type: ignore # typing is gay with instance bindings
    ...
    async def _star_message(
        ...
        *,
        ...
        guild_id = channel.guild.id
        starboard = await self.get_starboard(guild_id)  # type: ignore # typing is gay with instance bindings
        ...
        if channel.id == starboard_channel.id:
            ...
            await self._star_message(ch, record["message_id"], starrer_id, connection=connection)  # type: ignore # yeah this won't be the other channels
            return
        ...
        assert record is not None  # this is guarded just above
    ...
    async def unstar_message(
        self, channel: MessageableGuildChannel, message_id: int, starrer_id: int, *, verify: bool = False
    ) -> None:
        guild_id = channel.guild.id
        lock = self._locks.get(guild_id)
        if lock is None:
    ...
    async def _unstar_message(
        self, channel: MessageableGuildChannel, message_id: int, starrer_id: int, *, connection: asyncpg.Connection
    ) -> None:
        """Unstars a message.
        ...
        if channel.id == starboard_channel.id:
            ...
            return await self._unstar_message(ch, record["message_id"], starrer_id, connection=connection)  # type: ignore # yeah it'll never be those channels
        ...
        assert count is not None  # never gonna be none
        count = count[0]
    ...
    @checks.is_mod()
    async def starboard(self, ctx: Context, *, name="starboard") -> None:
        ...
        starboard = await self.get_starboard(ctx.guild.id, connection=ctx.db)  # type: ignore # typing is gay with instance bindings
        if starboard.channel is not None:
            await ctx.send(f"This server already has a starboard ({starboard.channel.mention}).")
            return
    ...
    @requires_starboard()
    async def starboard_info(self, ctx: Context) -> None:
    ...
    async def star(
        ...
        """
        assert isinstance(ctx.channel, (discord.TextChannel, discord.Thread, discord.VoiceChannel))

        try:
            await self.star_message(ctx.channel, message, ctx.author.id)
        except StarError as error:
            await ctx.send(str(error))
        else:
    ...
    @checks.is_mod()
    @requires_starboard()
    async def star_clean(self, ctx: Context, stars: int = 1) -> None:
        ...
        """
        assert ctx.starboard.channel is not None
        assert ctx.guild is not None

        stars = max(stars, 1)
        channel = ctx.starboard.channel

        last_messages = [message.id async for message in channel.history(limit=100)]
    ...
    async def star_show(self, ctx: Context, message: int if TYPE_CHECKING else MessageID) -> None:
        ...
        msg = await self.get_message(channel, record["message_id"])  # type: ignore # yeah it won't be those channel types
        if msg is None:
    ...
    @requires_starboard()
    @checks.is_mod()
    async def star_migrate(self, ctx: Context) -> None:
        ...
        perms = ctx.starboard.channel.permissions_for(ctx.guild.me)
        ...
        stats: Stats | None = self.bot.get_cog("Stats")  # type: ignore # yeah idk
        if stats is None:
            await ctx.send("Internal error occurred: Stats cog not loaded")
            return
    ...
    def records_to_value(
        self, records: list[asyncpg.Record], fmt: Callable[[str], str] | None = None, default: str = "None!"
    ) -> str:
        if not records:
            return default
    ...
    async def star_guild_stats(self, ctx: Context) -> None:
        assert ctx.guild is not None
        assert ctx.starboard.channel is not None
        ...
        assert record is not None  # can't ever be None
    ...
    async def star_member_stats(self, ctx: Context, member: discord.Member) -> None:
        assert ctx.guild is not None
    ...
    @requires_starboard()
    async def star_stats(self, ctx: Context, *, member: discord.Member | None = None) -> None:
    ...
    @requires_starboard()
    async def star_random(self, ctx: Context) -> None:
        ...
        if message is None:
            await ctx.send(f"Message {message_id} has been deleted somehow.")
            return
    ...
    @checks.is_mod()
    @requires_starboard()
    async def star_lock(self, ctx: Context) -> None:
        ...
        if ctx.starboard.needs_migration:
            await ctx.send("Your starboard requires migration!")
            return
    ...
    @checks.is_mod()
    @requires_starboard()
    async def star_unlock(self, ctx: Context) -> None:
    ...
    @checks.is_mod()
    @requires_starboard()
    async def star_limit(self, ctx: Context, stars: int) -> None:
    ...
    @checks.is_mod()
    @requires_starboard()
    async def star_age(self, ctx: Context, number: int, units: str = "days") -> None:
        ...
        if units not in valid_units:
            await ctx.send(f"Not a valid unit! I expect only {human_join(valid_units)}.")
            return

        number = min(max(number, 1), 35)

        if units == "years" and number > 10:
            await ctx.send("The maximum is 10 years!")
            return
    ...
    @commands.is_owner()
    async def star_announce(self, ctx: Context, *, message: str) -> None:
